...
#pragma once
#if !defined(__MITSUBA_RENDER_GKDTREE_MEDIAN_H_)
#define __MITSUBA_RENDER_GKDTREE_MEDIAN_H_
...
template <typename AABBType, typename TreeConstructionHeuristic, typename Derived>
	class GenericKDTreeMedian : public KDTreeBase<AABBType> {
protected:
...
public:
	 ...
	 */
	GenericKDTreeMedian() : m_indices(NULL) {
		m_nodes = NULL;
		m_traversalCost = 15;
		m_queryCost = 20;
...
protected:
	...
	struct RewriteItem {
		const KDNode *node;
		KDNode *target;
		const typename GenericKDTreeMedian::BuildContext *context;
		AABBType aabb;

		RewriteItem(const KDNode *node, KDNode *target,
			const typename GenericKDTreeMedian::BuildContext *context, const AABBType &aabb) :
...
protected:
	...
	public:
		TreeBuilder(IndexType id, GenericKDTreeMedian *parent)
	...
	Float buildTreeMinMax(BuildContext &ctx, unsigned int depth, KDNode *node,
		...
		if ( ( m_traversalCost + m_queryCost * (bestSplit.numLeft * 0.5f + bestSplit.numRight * 0.5f) ) >= leafCost) {
			createLeaf(ctx, node, indices, primCount);
			return leafCost;
		}
	...
	Float buildTree(BuildContext &ctx, unsigned int depth, KDNode *node,
		...
		Vector v = nodeAABB.getExtents();
		int bestAxis = v.x > v.y ? ( v.x > v.z ? 0 : 2 ) : ( v.y > v.z ? 1 : 2 );
			...
			if (EXPECT_TAKEN(pos > nodeAABB.min[axis] && pos < nodeAABB.max[axis])) {
				...
				if (numPlanar == 0) {
					Float cost = m_traversalCost + m_queryCost
						* std::abs(prob.first * nLF - prob.second * nRF);
					if (nL == 0 || nR == 0)
						cost *= m_emptySpaceBonus;
					if (cost < bestSplit.cost && bestAxis == axis) {
						bestSplit.pos = pos;
						bestSplit.axis = axis;
						bestSplit.cost = cost;
					...
					Float costPlanarLeft  = m_traversalCost
						+ m_queryCost * std::abs(prob.first * (nL+numPlanar) - prob.second * nRF);
					Float costPlanarRight = m_traversalCost
						+ m_queryCost * std::abs(prob.first * nLF - prob.second * (nR+numPlanar));
		...
		/* "Bad refines" heuristic from PBRT */
		if ((bestSplit.numLeft + bestSplit.numRight) > (1.8f * primCount)) {
			createLeaf(ctx, node, eventStart, eventEnd, primCount);
			return leafCost;
		}
			...
			for (int axis=0; axis<PointType::dim; ++axis) {
				...
				for (int i=0; i<m_binCount-1; ++i) {
					...
					Float cost = traversalCost + queryCost
						* std::abs(prob.first * numLeft - prob.second * numRight);

					if (cost < candidate.cost && axis == bestAxis) {
						candidate.cost = cost;
						candidate.axis = axis;
