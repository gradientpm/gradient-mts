...
GLWidget::GLWidget(QWidget *parent) :
    QGLWidget(parent), m_context(NULL), m_parent(parent) {
...
void GLWidget::saveSensor(SceneContext *ctx, const QString &targetFile) {
    // Make an empty document and insert only the sensor node
    QDomDocument newDoc;   
    QDomElement sensor = newDoc.createElement("sensor");
    newDoc.appendChild(sensor);
    
    // ====================================================================
    //   Serialize the sensor configuration
    // ====================================================================

    setProperties(newDoc, sensor, ctx->scene->getSensor()->getProperties());
    ...
    QDomElement sampler = newDoc.createElement("sampler");
    sensor.appendChild(sampler);

    setProperties(newDoc, sampler,
    ...
    QDomElement film = newDoc.createElement("film");
    ...
    setProperties(newDoc, film, filmProps);

    // ====================================================================
    //   Serialize the reconstruction filter configuration
    // ====================================================================

    QDomElement rfilter = newDoc.createElement("rfilter");
    film.appendChild(rfilter);

    setProperties(newDoc, rfilter,
    ...
    */
    ...
    for (int i=0; i<oldSensorReferences.size(); ++i)
        sensor.appendChild(newDoc.importNode(oldSensorReferences[i], true));

    QFile file;
    file.setFileName(targetFile);
    if (!file.open(QIODevice::WriteOnly | QIODevice::Truncate | QIODevice::Text)) {
        QMessageBox::critical(m_parent, m_parent->tr("Unable to save"),
            m_parent->tr("Unable to save changes: could not open the destination file <b>%1</b>.").arg(targetFile),
            QMessageBox::Ok);
        return;
    }
    ...
    QString textContent = newDoc.toString();
...
void GLWidget::keyPressEvent(QKeyEvent *event) {
    ...
    switch (event->key()) {
        ...
        case Qt::Key_F: {
            ...
            break;
        case Qt::Key_Space: {
                QString fileDir = QFileInfo(m_context->fileName).absolutePath();
                fileDir += "/viewpoints";                
                if (!QDir(fileDir).exists()) {
                    QDir().mkdir(fileDir);
                }
                static int viewpointCount = 0;      // TODO: detect from list
                m_statusMessage = "Save viewpoint " + viewpointCount;
                std::stringstream ss; 
                ss << "viewpoints/view" << viewpointCount << ".xml";
                saveSensor(m_context, ss.str().c_str());
                viewpointCount++;
            };
            break;
    }
