...
SamplingIntegrator::SamplingIntegrator(const Properties &props)
 : Integrator(props) {
	/**
	 * When this flag is set to true, texture filtering using ray differential is disabled.
	 * Ray differential is good for path tracing but the number of samples is needed to be known in advance.
	 * Moreover, texture filtering makes more difficult to compare to other rendering techniques (BDPT, PM, ...)
	 */
	m_noRayDiff = props.getBoolean("noRayDiff", true);
}
