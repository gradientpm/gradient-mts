        ...
        if (vertex_old->isSurfaceInteraction()) {
            ...
            if (vertex_old->isConnectable()) {
                ...
                if (!vertex->perturbDirection(m_scene,
                                              ...
                                              dist, succ_old->getType(), mode, PathVertex::OptionPerturbDirection{})) {
                    goto fail;
                }
            } else {
...
bool ManifoldPerturbation::generateOffsetPathGBDPT(Path &source, Path &proposal, MutationRecord &muRec, Vector2 offset,
                                                   bool &couldConnectBehindB, bool lightPath, bool merge_only) {
    int k = source.length();
    ...
    /* Generate subsequent vertices between a .. b deterministically */
    if (!propagatePerturbation(source, proposal, step, a, b, mode, false)) {
        goto fail;
    }
    ...
    if (merge_only) {
        ...
        for (int i = 0; i <= proposal.length(); i++) {
            ...
            if (proposal.vertex(i)->getType() == PathVertex::ESurfaceInteraction &&
                proposal.vertex(i)->componentType == 0) //BUG? this looks weird! (look at the second condition...)
...
bool ManifoldPerturbation::perturbDirection(Path &source, Path &proposal, int step, int a, Vector2 offset,
                                            ETransportMode mode, bool lightPath) {
    const PathVertex
            *pred_old = source.vertex(a - step),
            *vertex_old = source.vertex(a),
            *succ_old = source.vertex(a + step);
    ...
    if (!vertex->perturbDirection(m_scene,
                                  pred, predEdge, succEdge, succ, wo_new,
                                  succEdge_old->length, succ_old->getType(), mode, PathVertex::OptionPerturbDirection{})) {
        return false;
    }
...
bool
ManifoldPerturbation::propagatePerturbation(Path &source, Path &proposal, int step, int a, int b, ETransportMode mode,
                                            bool gvpm) {
    ...
    for (int i = a + step; i != b; i += step) {
        ...
        if (vertex_old->isSurfaceInteraction()) {
            ...
            if (gvpm) {
                vertex->sampledComponentIndex = vertex_old->sampledComponentIndex;
            }
            if (vertex_old->isConnectable()) {
                Vector m(0.0f);
                if (reflection) {
...
bool ManifoldPerturbation::manifoldWalk(const mitsuba::Path &source, mitsuba::Path &proposal, int step, int b, int c,
                                        bool updateAll, mitsuba::Float rs, mitsuba::Float rp) {
    /* Choose a local parameterization of the specular manifold using
    a plane with the following normal (which is computed in a
    reversible manner) */
    ...
    if (!m_manifold->move(vb_old->getPosition(), n)) { // rs
...
bool ManifoldPerturbation::manifoldWalkGPM(const Path &source, Path &proposal,
    ...
    if (!m_manifold->update(source, proposal, b, c, true, true)) {
        ++statsMWFailUpdate;
        return false;
    }
