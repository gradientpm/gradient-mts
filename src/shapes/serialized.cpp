	...
	SerializedMesh(const Properties &props) : TriMesh(props) {
		...
		*/
		...
		if (!objectToWorld.isIdentity()) {
            ...
            size_t nbInconsitentNormals = 0;
			if (m_normals) {
				for (size_t i=0; i<m_vertexCount; ++i) {
					m_normals[i] = normalize(objectToWorld(m_normals[i]));
                    nbInconsitentNormals += m_normals[i].lengthSquared() == 0.f ? 1 : 0;
				}
			}

            if(nbInconsitentNormals != 0) {
              SLog(EWarn, "Inconsistent normal when we load this object: %i issues", nbInconsitentNormals);
            }
		}
