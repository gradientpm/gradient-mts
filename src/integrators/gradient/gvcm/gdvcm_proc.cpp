...
public:
    ...
    void prepare() {
        ...
        if (m_process->phase == VCMProcessBase::EVAL) {
            m_offsetGenerator = new ManifoldPerturbation(m_scene, NULL, m_pool, 0.f, true, true, 0, 0,
                    m_config.m_shiftThreshold, m_config.maxManifoldIterations);
        }
    }
    ...
    void evaluateMerging(GDVCMWorkResult *wr,
                ...
                if (pathSuccess[k] && pathSuccess[0] && (k == 0 || (valuePdf[0] > 0 && !value[0].isZero()))) {
                    ...
                    if (prev_conn) {
                        ...
                        if (k == 0) {
                            ...
                            value[0] = merge_prob == 0 ? Spectrum(0.0) : importanceWeights[s + 1] * vtEval *
                            ...
                            value[k] *= merge_prob == 0 ? Spectrum(0.f) : geomTerm / p_acc / merge_prob * vs_conn->rrWeight;
                            valuePdf[k] = *importancePdfTmp * *radiancePdfTmp;
                        }
                    } else {
                        // unconnectable case
                        // prefix-suffix weights + connection bsdfs
                        value[k] = merge_prob == 0 ? Spectrum(0.0) : *importanceWeightTmp * *radianceWeightTmp * vtEval
        ...
        Spectrum mainRad = valuePdf[0] * miWeight[0] * value[0];
        if (hasNaN(mainRad)) mainRad = Spectrum(0.f);
            primal += mainRad;
        ...
        for (int n = 0; n < neighbourCount; n++) {
            ...
            Spectrum gradVal = Float(2.f) * miWeight[n + 1] * (fy - fx);
            if (hasNaN(gradVal)) gradVal = Spectrum(0.f);
#if defined(DEBUG_MERGING)
    ...
    inline bool hasNaN(const Spectrum& spec) {
        for (int i=0; i<SPECTRUM_SAMPLES; i++)
            if(spec[i] != spec[i]) {
                SLog(EError, "Nan Value dectected");
                return true;
            }
        return false;
    }
...
void GDVCMProcess::develop() {
    ...
    for (int i = 0; i <= m_config.nNeighbours; ++i) {
        ...
        m_film->addBitmapMulti(m_result->getLightImage(i)->getBitmap(), m_weight / sampleCount, i);
    }
...
void GDVCMProcess::processResult(const WorkResult *wr, bool cancelled) {
    ...
    if (m_config.lightImage) {
        ...
        if (m_parent->isInteractive()) {
            ...
            Float invSampleCount = 1.0f / sampleCount * m_weight;
