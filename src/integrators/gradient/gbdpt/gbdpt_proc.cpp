...
public:
    ...
    ref<WorkResult> createWorkResult() const {
        return new GBDPTWorkResult(m_config, m_rfilter.get(), Vector2i(m_config.blockSize),
                                   m_config.nNeighbours/*, m_config.nFeatures*/, m_config.extraBorder);
    }
    ...
    void process(const WorkUnit *workUnit, WorkResult *workResult, const bool &stop) {
        ...
        for (size_t i = 0; i < m_hilbertCurve.getPointCount(); ++i) {
            ...
            for (size_t j = 0; j < spp; j++) {
                ...
                createShiftablePath(connectPath, emitterSubpath, sensorSubpath[0], 1,
                                    sensorSubpath[0].vertexCount() - 1, ptx);
                    ...
                    if (pathData[k + 1].success) {
                        ...
                        for (int v = pathData[k + 1].muRec.extra[0] - 1; v >= 0; v--) {
                            ...
                            if (Path::isConnectable_GBDPT(connectPath.vertex(v), m_config.m_shiftThreshold) &&
                                ...
                                pathData[k + 1].genGeomTerm.at(idx) = sensorSubpath[k + 1].calcSpecularPDFChange(v,
                                                                                                                 m_offsetGenerator);
                            } else {
    ...
    void evaluate(GBDPTWorkResult *wr,
                  Path &emitterSubpath, std::vector<Path> &sensorSubpath, std::vector<ShiftPathData> &pathData,
                  int vert_b,
            ...
            for (int t = minT; t <= maxT; ++t) {
                ...
                if (isDirect) {
                    ...
                    for (int tLocal = t - 1; tLocal >= minT; tLocal--) {
                        if (t < 2) continue;
                        isDirect &= !(sensorSubpath[0].vertex(tLocal)->isConnectable());
                    }
                }
#endif
                samplePosValid = true;
                ...
                for (int k = 0; k <= neighbourCount; k++) {
                    ...
                    if (t == 1 && k == 0) {
                        pathSuccess[0] = createShiftablePath(connectedBasePath, emitterSubpath, sensorSubpath[0], s, 1,
                                                             memPointer);
                        m_offsetGenerator->computeMuRec(connectedBasePath, muRec);
                        genGeomTermLP[0] = connectedBasePath.calcSpecularPDFChange(muRec.extra[2], m_offsetGenerator,
                                                                                   true);
                    ...
                    if (t == 1 && k > 0 && !value[0].isZero()) {
                        if (!pathSuccess[0])
                            ...
                            createShiftedLightPath(connectedBasePath, offsetEmitterSubpath, jacobianLP[k - 1],
                                                   pathSuccess[k], offsetImportanceWeight, offsetImportancePdf, muRec,
                                                   samplePos, shifts[k - 1]/*nIdxL[k - 1]*/, s);
                            if (pathSuccess[k]) {
                                genGeomTermLP[k] = offsetEmitterSubpath.calcSpecularPDFChange(muRec.extra[2],
                                                                                              m_offsetGenerator, true);
                    ...
                    do { //this should really go away at some point...
                        ...
                        if (pathSuccess[k] && pathSuccess[0] && (k == 0 || (valuePdf[0] > 0 && !value[0].isZero()))) {
                            ...
                            bool successConnect = (k > 0 && t > vert_b) ? successConnectBase
                                                                        : connectionEdge.pathConnectAndCollapse(scene,
                                                                                                                vsEdge,
                                                                                                                vs, vt,
                                                                                                                vtEdge,
                                                                                                                interactions);
                            ...
                            geomTerm = (k > 0 && t > vert_b) ? geomTermBase : connectionEdge.evalCached(vs, vt,
                            ...
                            if (k == 0) {
                                ...
                                miWeight[0] =
                                        Path::miWeightBaseNoSweep_GBDPT(scene, emitterSubpath, &connectionEdgeBase,
                                                                        sensorSubpath[0],
                                                                        *emitterSubpathTmp, &connectionEdge,
                                                                        *sensorSubpathTmp, s, t, m_config.sampleDirect,
                                                                        m_config.lightImage,
                                                                        1.0, 1.0, (t < 2 ? genGeomTermLP[0]
                                                                                         : pathData[0].genGeomTerm[t]),
                                                                        0.f, vert_b, m_config.m_shiftThreshold) /
                                        valuePdf[0];
                            } else {
                                ...
                                miWeight[k] =
                                        Path::miWeightGradNoSweep_GBDPT(scene, emitterSubpath, &connectionEdgeBase,
                                                                        sensorSubpath[0],
                                                                        *emitterSubpathTmp, &connectionEdge,
                                                                        *sensorSubpathTmp, s, t, m_config.sampleDirect,
                                                                        m_config.lightImage,
                                                                        (t < 2 ? jacobianLP[k - 1]
                                                                               : pathData[k].jacobianDet[t]), 1.0,
                                                                        (t < 2 ? genGeomTermLP[0]
                                                                               : pathData[0].genGeomTerm[t]),
                                                                        (t < 2 ? genGeomTermLP[k]
                                                                               : pathData[k].genGeomTerm[t]),
...
private:
    ...
    void combineImportanceData(const Path &path, Spectrum *weights, Float *pdf) {
        ...
        for (size_t i = 1; i < path.vertexCount(); ++i) {
            weights[i] = weights[i - 1] * path.vertex(i - 1)->weight[EImportance] * path.vertex(i - 1)->rrWeight *
                         path.edge(i - 1)->weight[EImportance];
            pdf[i] = pdf[i - 1] * path.vertex(i - 1)->pdf[EImportance] * path.vertex(i - 1)->rrWeight *
                     path.edge(i - 1)->pdf[EImportance];
        }
    }

    /* Compute radiance data from the emitter path and store it in a easy to access way */
    void combineRadianceData(const std::vector<Path> path, std::vector<ShiftPathData> pathData, const int neighbours,
                             Spectrum **weights, Float **pdf) {
        for (int k = 0; k <= neighbours; k++) {
            weights[k][0] = Spectrum(1.0f);
            pdf[k][0] = Float(1.0);
            for (size_t i = 1; i < path[0].vertexCount(); ++i) {
                if (pathData[k].success && i < path[k].vertexCount()) {
                    weights[k][i] = weights[k][i - 1] * path[k].vertex(i - 1)->weight[ERadiance] *
                                    path[k].vertex(i - 1)->rrWeight * path[k].edge(i - 1)->weight[ERadiance];
                    pdf[k][i] =
                            pdf[k][i - 1] * path[k].vertex(i - 1)->pdf[ERadiance] * path[k].vertex(i - 1)->rrWeight *
                            path[k].edge(i - 1)->pdf[ERadiance];
                }
            }
        }
    }
    ...
    void createShiftedLightPath(Path &base, Path &offset, double &jacobian, bool &pathSuccess, Spectrum &offsetWeight,
                                Float &offsetPdf, MutationRecord &muRec,
        ...
        pathSuccess = m_offsetGenerator->generateOffsetPathGBDPT(base, offset, muRec, shiftOffset, couldConnectWithB,
                                                                 true);
        ...
        if (pathSuccess) {
            ...
            for (size_t i = 1; i <= s; ++i) {
                offsetWeight =
                        offsetWeight * offset.vertex(i - 1)->weight[EImportance] * offset.vertex(i - 1)->rrWeight *
                        offset.edge(i - 1)->weight[EImportance];
                offsetPdf = offsetPdf * offset.vertex(i - 1)->pdf[EImportance] * offset.vertex(i - 1)->rrWeight *
                            offset.edge(i - 1)->pdf[EImportance];
            }
        }
    }
    ...
    **/
    bool
    createShiftablePath(Path &connectedPath, Path &emitterSubpath, Path &sensorSubpath, int s, int t, int &memPointer) {
        connectedPath.clear();
        ...
        if (t == 1)
            connectedPath.vertex(connectedPath.vertexCount() - 2)->updateSamplePosition(
                    connectedPath.vertex(connectedPath.vertexCount() - 3));
...
void GBDPTProcess::develop() {
    ...
    for (size_t i = 0; i <= m_config.nNeighbours; ++i) {
        m_film->setBitmapMulti(
                m_result->getImageBlock(i)->getBitmap()->crop(Point2i(m_config.extraBorder), m_film->getCropSize()),
                Float(1.0), i);
...
void GBDPTProcess::bindResource(const std::string &name, int id) {
    ...
    if (name == "sensor" && m_config.lightImage) {
        ...
        m_result = new GBDPTWorkResult(m_config, NULL, m_film->getCropSize(), m_config.nNeighbours,
                                       m_config.extraBorder);
        m_result->clear();
    }
}
