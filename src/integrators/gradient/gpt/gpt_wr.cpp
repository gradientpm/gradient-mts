...
GPTWorkResult::GPTWorkResult(const ReconstructionFilter *rfilter, bool computeVariance, Vector2i blockSize, int extraBorder) {
	if (blockSize == Vector2i(-1, -1))
		blockSize = Vector2i(32, 32);


	const size_t BUFFER_COUNT = computeVariance ? 5 + 3 : 5;
	m_block.resize(BUFFER_COUNT); // 0: preview, 1: throughput, 2: dx, 3: dy, 4: very direct
	// if variance: th2, dx2 and dy2
	...
	// Variance buffer do not need

	if(computeVariance) {
		// FIXME: Normally this does not change anything
		m_block[5]->setAllowNegativeValues(true); // dx
		m_block[6]->setAllowNegativeValues(true); // dy
		m_block[7]->setAllowNegativeValues(true); // dy
	}
}
