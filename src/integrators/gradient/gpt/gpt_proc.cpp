...
public:
	GPTBlockRenderer(Bitmap::EPixelFormat pixelFormat, int channelCount, int blockSize,
		int borderSize, bool warnInvalid, const GradientPathTracerConfig& config) :
	...
	void process(const WorkUnit *workUnit, WorkResult *workResult,
		...
		m_integrator->renderBlock(m_scene, m_sensor, m_sampler,
			block, stop, m_hilbertCurve.getPoints(), rect->getSize());
	...
	ref<WorkProcessor> clone() const override {
		return new GPTBlockRenderer(m_pixelFormat, m_channelCount,
			m_blockSize, m_borderSize, m_warnInvalid, m_config);
	}
...
ref<WorkProcessor> GPTRenderProcess::createWorkProcessor() const {
	return new GPTBlockRenderer(m_pixelFormat, m_channelCount, m_blockSize, m_borderSize, m_warnInvalid, m_config);
}
